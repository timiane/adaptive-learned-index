...
bool KerasModel::LoadModel(const std::string& filename) {
    ...
    for (unsigned int i = 0; i < num_layers; i++) {
        ...
        KASSERT(ReadUnsignedInt(&file, &layer_type), "Expected Stage type");
        ...
        KASSERT(layer, "Unknown Stage type %d", layer_type);

        bool result = layer->LoadLayer(&file);
        if (!result) {
            printf("Failed to load Stage %d", i);
            delete layer;
            return false;
        }
