...
lognorm <- read.csv("/home/jesper/learnedIndexes/li/data/log_norm.csv",
                    header = FALSE)
result1 <- data.frame(x = 1:2000000)
lognorm = lognorm[order(lognorm$V1), ]
...
#-----------------------------------------------------------------#
#   11 - LR Distance of the Data Itens to the Linear Function     #
...
#       |yi ÃÂ¢ÃÂÃÂ yj|) and creates a new random test point by randomly#
...
createTree = function(dataset, output=NULL) {
  formatedDataset = FormatDataset(dataset, output)
  model <- tree(formatedDataset$input)
  plot(model)
  text(model)
}
...
autoMeta = function(){
  filenames <- list.files("datatwo/", pattern = "*.csv", full.names = TRUE)
  headers <- list.files("datatwo/", pattern = "*.csv", full.names = FALSE)
  ...
  write.csv(output, file = "metaFeatureForMetaLearner.csv", sep = ",", row.names = FALSE, col.names = FALSE, quote = FALSE)
}
setwd("~/Code/ARR")
