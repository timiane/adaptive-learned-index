...
titles = function(header, output = NULL){
  ...
  if(grepl("logn", header)){
    result = "logn"}
  if(grepl("logar", header)){
    result = "logar"}
  if(grepl("sinl", header)){
    result = "sinl"}
  if(grepl("zipf", header)){
    result = "zipf"}
  if(grepl("power", header)){
    result = "power"}
...
autoMeta = function(dataset){
    result <- data.frame(x = 1:length(dataset))
    resultForPython <- data.frame(
                       LR = LR(dataset, result),
                       #OEID = OIED(dataset, result),
                       SNNR = SNNR(dataset, result),
                       INST = Inst(dataset, result),
                       VAR = variance(dataset, result),
                       DD =deltaDifference(dataset, result), 
                       KURT = kurt(dataset, result),
                       CORR = correlation(dataset, result))
}
resultForPython = autoMeta(args)
