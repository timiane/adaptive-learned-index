...
def load_data():
    ...
    # data_frame = data_frame.rename({'0':'name', '1':'training_time'})
    data_frame['size'] = 0
...
def extract_times(current_dataset):
    ...
    data_set = np.loadtxt('datasets/' + get_dataset_name(name) + ".csv")
...
def evaluate_time(data_frame):
    ...
    data_frame.to_csv('times.csv', index=None)


def load_data_frame():
    return pd.DataFrame.from_csv('times.csv')
...
def calculate_aar(data_frame, size_weight, time_weight, accuracy_weigt):
    ...
    for name in data_set_names:
        ...
        # dictionary['lasso'] = data_frame.loc['lasso_' + name, :]
        ...
        data_frame_result.loc[name] = Arr.weighted_sum_method(data_frame_benifitial, data_frame_non_benifitial, weights)
    data_frame_result.to_csv('experiments/aar_test_k_' + '[' + str(size_weight) + ',' + str(time_weight) + ',' + str(accuracy_weigt) + ']')
    return data_frame_result


def print_boxplot(size, time, accuracy):
    aar = pd.DataFrame.from_csv('experiments/aar_test_k_[' + str(size) + ',' + str(time) + ',' + str(accuracy) + ']')
...
if __name__ == '__main__':
    ...
    calculate_aar(loaded_data_frame, 0.4, 0.2, 0.4)
